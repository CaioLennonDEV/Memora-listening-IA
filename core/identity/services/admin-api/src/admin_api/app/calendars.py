...
def default_bot_name() -> str:
    """Deployment default display name for the bot in the room.

    Reads ``DEFAULT_BOT_NAME`` at call time so an operator can rename without a rebuild.
    Falls back to ``Vexa`` when unset (open-core default).
    """
    return (os.getenv("DEFAULT_BOT_NAME") or "").strip() or "Vexa"
...
def connections_from_data(data: dict, user_id: int, *, include_deleted: bool = False) -> list[dict]:
    ...
    if isinstance(raw, list):
        ...
        connections = ([{
            ...
            "bot_name": data.get("calendar_bot_name") or default_bot_name(),
            "enabled": True,
        }] if legacy_url else [])
...
def new_connection(*, name: str, ics_url: str, auto_join: bool = True,
    ...
    return {
        ...
        "bot_name": validate_bot_name(bot_name or default_bot_name()),
        "enabled": True,
    }
...
def masked_connection(connection: dict) -> dict:
    ...
    return {
        ...
        "bot_name": connection.get("bot_name") or default_bot_name(),
