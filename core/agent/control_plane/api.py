    ...
    *,
    ...
    def meeting_stream(meeting_id: str, session_uid: str, request: Request):
        ...
        def gen():
            ...
            def roster_event(payload):
                """Live observed roster (Wave 1) → `roster` SSE for Terminal `N in the room`."""
                yield ({
                    "type": "roster",
                    "tMs": payload.get("tMs"),
                    "reliable": bool(payload.get("reliable")),
                    "humans": int(payload.get("humans") or 0),
            ...
            if resume_t is None:   # fresh connect → seed the bounded recent transcript tail
                ...
                for entry_id, fields in seed_rows:
                    ...
                    if payload.get("type") == "retract":
                        ...
                        continue
                    if payload.get("type") == "roster":
                        yield from roster_event(payload)
                        continue
