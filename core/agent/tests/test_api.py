...
class _RosterRedis:
    """Live tail delivers a roster snapshot for Terminal `N in the room`."""
    def __init__(self):
        self._reads = 0
    ...
    def xread(self, last, count=500, block=0):
        ...
        if self._reads == 1:
            return [("tc:meeting:m1", [("1-0", {"payload": _j.dumps({
                "type": "roster", "tMs": 99, "reliable": True, "humans": 2,
                "people": [{"id": "a", "name": "Alice"}, {"id": "b", "name": "Bob"}],
            })})])]
        return []


def test_sse_relays_roster_event(monkeypatch):
    """A roster payload on tc:meeting relays as a `roster` SSE event with humans count."""
    fr = _RosterRedis()
    c = _stream_client(fr, monkeypatch)
    with c.stream("GET", "/api/meeting/stream", params={"meeting_id": "m1", "session_uid": "m1"},
                  headers={"X-User-Id": "u_owner"}) as r:
        assert r.status_code == 200
        body = "".join(r.iter_text())
    assert '"type": "roster"' in body or '"type":"roster"' in body
    assert '"humans": 2' in body or '"humans":2' in body
