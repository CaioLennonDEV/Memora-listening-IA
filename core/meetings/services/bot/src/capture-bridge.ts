...
export async function startCaptureBridge(
  ...
  /** Google Meet live roster (Wave 1) — tile scan → Node; publish + alone-leave. */
  onRoster?: (snapshot: {
    tMs: number;
  ...
  // gmeet live roster → composition root (publish + alone-leave debounce).
  await page.exposeFunction('__vexaRoster', (snapshot: {
    tMs: number;
    people: { id: string; name?: string; isSelf: boolean }[];
    reliable: boolean;
  }): void => {
    try { onRoster?.(snapshot); } catch (e) { console.error(`[bot] roster sink rejected: ${String(e)}`); }
  }).catch(() => { /* optional */ });
    ...
    if (isMixed) {
      ...
      if (isZoom && w.VexaBrowserUtils?.createZoomSpeakers && !w.__vexaZoomSpeakers) {
        ...
        w.__vexaZoomSpeakers = w.VexaBrowserUtils.createZoomSpeakers({
          ...
          onRoster: (snapshot: unknown) => w.__vexaRoster?.(snapshot),
        });
      }
      return;
      }
