...
import { createRosterPublisher } from './adapters/roster-redis.js';
...
export async function main(env: NodeJS.ProcessEnv = process.env): Promise<number> {
  ...
  const rosterAloneDebounceMs = resolveRosterAloneDebounceMs(env);
  ...
  const silenceAloneness = createSilenceAlonenessSource({
    activity: remoteAudioActivity,
    windowMs: aloneSilenceWindowMs,
    onCaptureFault: () => restartCapture?.(),
  });
  const rosterAloneness = createRosterAlonenessSource({ debounceMs: rosterAloneDebounceMs });
  const aloneness = combineAloneness(silenceAloneness, rosterAloneness);
  const rosterPublisher = createRosterPublisher({
    client: transcriptClient,
    meetingId,
    nativeMeetingId: inv.nativeMeetingId,
  });
  let lastRosterFp = '';
  const onRoster = (snapshot: RosterSnapshot): void => {
    rosterAloneness.observe(snapshot);
    const humans = humanCount(snapshot);
    const fp = `${snapshot.reliable}:${humans ?? 'u'}:${snapshot.people.map((p) => p.id).sort().join(',')}`;
    if (fp === lastRosterFp) return;
    lastRosterFp = fp;
    void rosterPublisher.publish({
      ...snapshot,
      humans: humans ?? 0,
    }).catch((e) => console.error(`[bot] roster publish rejected: ${String(e)}`));
  };
  console.log(`[bot] aloneness: silence adapter + deaf-capture guard enabled (window_ms=${aloneSilenceWindowMs}); roster alone debounce_ms=${rosterAloneDebounceMs}`);
  ...
  try {
    ...
    pipeline = createLivePipeline({
      startCapture: () => startCaptureBridge(sess.page, inv, bp, signalRecorder?.sink, publishChat, remoteAudioActivity, onRoster),   // on the live meeting page
