...
async def test_ingest_roster_forwards_to_row_keyed_feed(store, bus):
    """Wave 1 live roster — not persisted as transcript; lands on tc:meeting:{row} for SSE."""
    n = await ingest(store, bus, {"payload": json.dumps({
        "type": "roster",
        "meeting_id": "1",
        "tMs": 123,
        "reliable": True,
        "humans": 2,
        "people": [{"id": "a", "name": "Alice"}, {"id": "c"}],
    })})
    assert n == 0
    # Must not touch the durable transcript store
    doc = await store.get_transcript(7, "google_meet", "abc-defg-hij")
    assert doc["segments"] == []
    rows = await bus._client.xrange("tc:meeting:1")
    assert len(rows) == 1
    raw = rows[0][1].get(b"payload") or rows[0][1].get("payload")
    payload = json.loads(raw.decode() if isinstance(raw, bytes) else raw)
    assert payload["type"] == "roster"
    assert payload["humans"] == 2
