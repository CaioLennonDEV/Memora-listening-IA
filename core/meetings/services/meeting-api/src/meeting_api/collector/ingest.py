...
async def ingest(store: TranscriptStore, redis: RedisBus, message: dict) -> int:
    ...
    if msg_type == "transcript_retract":
        ...
        return 0
    if msg_type == "roster":
        # Live observed roster (Wave 1) — not a transcript segment. Forward onto the row-scoped feed
        # so agent-api SSE / Terminal can show "N in the room". Never persist to the transcript store.
        mid_raw = data.get("meeting_id")
        try:
            meeting_id = int(mid_raw) if mid_raw is not None else None
        except (TypeError, ValueError):
        ...
        if meeting_id is None:
            return 0
        try:
            humans = int(data.get("humans") or 0)
        except (TypeError, ValueError):
            humans = 0
        out = {
            "type": "roster",
        ...
        try:
            await redis.xadd(_transcript_stream(meeting_id), out)
        except Exception as e:  # noqa: BLE001 — best-effort; never abort the batch
            _log_publish_failure(meeting_id, e)
        return 0
