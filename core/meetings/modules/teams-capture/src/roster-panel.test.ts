...
}
{
  ...
  check('open panel reports present=true', state.present === true);
}
{
  ...
  let present = true;
  try {
    names = readTeamsRosterPanel(empty);
    present = readTeamsRosterPanelState(empty).present;
  } catch { threw = true; }
  check('a closed panel yields nothing and never throws', !threw && names.length === 0, JSON.stringify(names));
  check('closed panel reports present=false', present === false);
}
{
