...
export interface TeamsRosterPanelState {
  ...
  /** true when at least one roster panel container was found (open), even if every row is self. */
  present: boolean;
}
...
export function readTeamsRosterPanelState(
  ...
  for (const panelSel of teamsRosterPanelSelectors) {
    ...
    if (panels.length > 0) present = true;
...
/** One person for the Wave 2 live-roster contract (isomorphic with bot `RosterPerson`). */
export type TeamsRosterPerson = { id: string; name?: string; isSelf: boolean };

/** Live roster snapshot (isomorphic with bot `RosterSnapshot`). */
export type TeamsRosterSnapshot = {
  tMs: number;
  people: TeamsRosterPerson[];
  reliable: boolean;
};

/**
 * Map Teams tile surfaces + optional open roster panel → live roster snapshot.
 *
 * `panelPeople === null` means the panel was not present (closed / absent). An empty array with
 * panel present means "only the bot remains" (self rows are filtered by the panel reader) and is
 * still reliable. Zero tiles AND no panel ⇒ unreliable (unknown ≠ empty).
 */
export function rosterFromTeamsPresence(opts: {
  tMs: number;
  surfaces: TeamsRosterPerson[];
  /** null = panel not in DOM; [] = panel open with no non-self rows */
  panelPeople: TeamsRosterPerson[] | null;
}): TeamsRosterSnapshot {
  const { tMs, surfaces, panelPeople } = opts;
  const reliable = surfaces.length > 0 || panelPeople !== null;
  const byKey = new Map<string, TeamsRosterPerson>();
  const keyOf = (p: TeamsRosterPerson): string => {
    if (p.isSelf) return `self:${normalizeDisplayNameForIdentity(p.name || p.id)}`;
    const n = normalizeDisplayNameForIdentity(p.name || '');
    return n || `id:${p.id}`;
  };
  for (const p of surfaces) byKey.set(keyOf(p), p);
  if (panelPeople) {
    for (const p of panelPeople) {
      const k = keyOf(p);
      const prev = byKey.get(k);
      if (!prev) byKey.set(k, p);
      else if (!prev.name && p.name) byKey.set(k, { ...prev, name: p.name });
    }
  }
  return { tMs, reliable, people: [...byKey.values()] };
}

/**
 * Wave 2 primary presence scan: every Teams stream wrapper carries the display name in `data-tid`.
 * Gallery / speaker view often drops non-active VIDEO TILES, but stream wrappers for connected
 * participants usually remain — this is why a human can be "in the call" while the tile walk
 * only sees the bot.
 *
 * Uses the stream tid as a name-authoritative surface (same latitude as the open roster panel).
 */
export function scanTeamsStreamRoster(
  root: ParentNode = document,
  selfName?: string,
): TeamsRosterPerson[] {
  const out: TeamsRosterPerson[] = [];
  const seen = new Set<string>();
  let nodes: Element[] = [];
  try { nodes = Array.from(root.querySelectorAll('[data-stream-type][data-tid]')); }
  catch { return out; }
  for (const node of nodes) {
    if (!(node instanceof HTMLElement)) continue;
    const tid = (node.getAttribute('data-tid') || '').trim();
    if (!tid) continue;
    const self = isSelfDisplayName(tid, selfName)
      || isSelfParticipantSurface(node, selfName);
    // Stream data-tid is Teams' participant label — admit bare handles like the roster panel.
    const okName = self || isTeamsDisplayNameCandidate(tid, { rosterAuthoritative: true });
    if (!okName) continue;
    const key = self
      ? `self:${normalizeDisplayNameForIdentity(tid)}`
      : (normalizeDisplayNameForIdentity(tid) || tid);
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({
      id: self
        ? `self:${normalizeDisplayNameForIdentity(tid)}`
        : `stream:${normalizeDisplayNameForIdentity(tid) || tid}`,
      name: tid,
      isSelf: self,
    });
  }
...
export interface TeamsSpeakersOptions {
  ...
  /** Live observed roster each scan (Wave 2 — true presence). */
  onRoster?: (snapshot: TeamsRosterSnapshot) => void;
  log?: (msg: string) => void;
...
export function createTeamsSpeakers(opts: TeamsSpeakersOptions): TeamsSpeakers {
  ...
  function emitLiveRoster(
    tileSurfaces: TeamsRosterPerson[],
    panelState: TeamsRosterPanelState,
  ): void {
    // Presence atom = each [data-stream-type][data-tid], NOT the canonicalized tile root.
    // A shared stable participant root can nest both the bot and a human stream; walking the
    // canonical surface then sees the bot's data-tid in the subtree and marks everyone as self —
    // which is exactly "Terminal shows only the bot while a human is still in the call".
    let streamPeople: TeamsRosterPerson[] = [];
    try { streamPeople = scanTeamsStreamRoster(document, opts.selfName); }
    catch { streamPeople = []; }
    const panelPeople: TeamsRosterPerson[] | null = panelState.present
      ? panelState.entries.map((name, i) => ({
          id: name ? `roster:${normalizeDisplayNameForIdentity(name)}` : `roster:unresolved:${i}`,
          name: name || undefined,
          isSelf: false,
        }))
      : null;
    try {
      const snap = rosterFromTeamsPresence({
        tMs: now(),
        surfaces: [...streamPeople, ...tileSurfaces],
        panelPeople,
      });
      opts.onRoster?.(snap);
      const humans = snap.people.reduce((n, p) => n + (p.isSelf ? 0 : 1), 0);
      const fp = `${streamPeople.length}|${tileSurfaces.length}|${panelState.present ? panelState.entries.length : 'x'}|${snap.reliable ? humans : '-'}|${snap.reliable}`;
      if (fp !== lastRosterLogFp) {
        lastRosterLogFp = fp;
        log(
          `[TeamsSpeakers] roster stream=${streamPeople.length} tiles=${tileSurfaces.length} `
          + `panel=${panelState.present ? panelState.entries.length : 'closed'} `
          + `humans=${snap.reliable ? humans : '—'} reliable=${snap.reliable}`,
        );
      }
  ...
  function scanAndObserveAll(): void {
    ...
    const rosterSurfaces: TeamsRosterPerson[] = [];
    ...
    let panelState: TeamsRosterPanelState = { names: [], entries: [], present: false };
    try { panelState = readTeamsRosterPanelState(document, { selfName: opts.selfName }); }
    catch { panelState = { names: [], entries: [], present: false }; }
    ...
    for (const selector of allSelectors) {
      document.querySelectorAll(selector).forEach(el => {
        if (el instanceof HTMLElement) {
          ...
          const self = isSelfParticipantSurface(surface, opts.selfName);
          if (self) {
            const selfName = extractTeamsSpeakerName(surface, { nameContext }) || opts.selfName || 'self';
            const id = extractId(surface);
            rosterSurfaces.push({ id, name: selfName, isSelf: true });
          } else {
    ...
    // Live roster (Wave 2) — true presence for Terminal + alone-leave.
    emitLiveRoster(rosterSurfaces, panelState);
  ...
  // Presence must move faster than the 5s speaker rescan — speaker view drops inactive tiles, and
  // stream wrappers / an open panel can appear or change between speaking rescans.
  const rosterInterval = setInterval(() => {
    if (destroyed) return;
    let panelState: TeamsRosterPanelState = { names: [], entries: [], present: false };
    try { panelState = readTeamsRosterPanelState(document, { selfName: opts.selfName }); }
    catch { panelState = { names: [], entries: [], present: false }; }
    emitLiveRoster([], panelState);
