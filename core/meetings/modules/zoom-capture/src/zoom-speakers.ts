...
export interface ZoomSpeakersOptions {
  ...
  /** Live observed roster each poll (Wave 3 — true presence from name footers). */
  onRoster?: (snapshot: ZoomRosterSnapshot) => void;
  log?: (msg: string) => void;
...
/** One person for the Wave 3 live-roster contract (isomorphic with bot `RosterPerson`). */
export type ZoomRosterPerson = { id: string; name?: string; isSelf: boolean };

/** Live roster snapshot (isomorphic with bot `RosterSnapshot`). */
export type ZoomRosterSnapshot = {
  tMs: number;
  people: ZoomRosterPerson[];
  reliable: boolean;
};

/** Normalize a Zoom footer display name for identity / dedupe. */
export function normalizeZoomDisplayName(value: string): string {
  return value.replace(/\s+/g, ' ').trim().toLowerCase();
}

/**
 * Map Zoom name-footer tiles → live roster. Zero named footers = unreliable
 * (DOM not ready / selectors drifted), not an empty room.
 */
export function rosterFromZoomTiles(
  tiles: { name: string }[],
  tMs: number,
  selfName?: string,
): ZoomRosterSnapshot {
  const selfNorm = selfName ? normalizeZoomDisplayName(selfName) : '';
  const people: ZoomRosterPerson[] = [];
  const seen = new Set<string>();
  for (const t of tiles) {
    const raw = (t.name || '').replace(/\s+/g, ' ').trim();
    if (!raw) continue;
    const norm = normalizeZoomDisplayName(raw);
    if (seen.has(norm)) continue;
    seen.add(norm);
    const isSelf = !!selfNorm && norm === selfNorm;
    people.push({ id: `zoom:${norm}`, name: raw, isSelf });
  }
  return { tMs, reliable: people.length > 0, people };
}

/** Scan Zoom's avatar name footers (the same surface getState().tiles uses). */
export function scanZoomNameTiles(root: ParentNode = document): { name: string }[] {
  const tiles: { name: string }[] = [];
  const footers = root.querySelectorAll(NAME_FOOTER_SELECTOR);
  const seen = new Set<string>();
  for (let i = 0; i < footers.length && tiles.length < 40; i++) {
    const footer = footers[i] as HTMLElement;
    const raw = (footer.querySelector('span')?.textContent?.trim() || footer.innerText?.trim() || '')
      .replace(/\s+/g, ' ')
      .trim();
    if (!raw) continue;
    const key = normalizeZoomDisplayName(raw);
...
export function createZoomSpeakers(opts: ZoomSpeakersOptions = {}): ZoomSpeakers {
  ...
  function tick(): void {
    ...
    // Live roster (Wave 3) — name footers = true presence (silent included).
    try {
      opts.onRoster?.(rosterFromZoomTiles(scanZoomNameTiles(), Date.now(), opts.selfName));
