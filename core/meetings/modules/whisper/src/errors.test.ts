...
async function run() {
  ...
  // Hosted external STT requires Idempotency-Key (1–255 chars) on every POST.
  {
    let seenKey = '';
    (globalThis as any).fetch = async (_url: unknown, init?: { headers?: Record<string, string> }) => {
      seenKey = init?.headers?.['Idempotency-Key'] ?? '';
      return new Response(JSON.stringify({ text: 'hi', language: 'en', duration: 0.1, segments: [] }), { status: 200 });
    };
    const client = new TranscriptionClient({ serviceUrl: 'http://stt.test' });
    await client.transcribe(pcm, 'en');
    check(
      'POST sends Idempotency-Key (1–255 chars)',
      typeof seenKey === 'string' && seenKey.length >= 1 && seenKey.length <= 255,
      `key=${seenKey}`,
    );
  }
