...
export interface GmeetSpeakersOptions {
  ...
  /** Live tile roster each poll (Wave 1 — true presence). Empty tiles ⇒ unreliable. */
  onRoster?: (snapshot: GmeetRosterSnapshot) => void;
...
/** Roster shape shared with the bot alone-leave / Terminal live path (isomorphic with bot RosterSnapshot). */
export type GmeetRosterSnapshot = {
  tMs: number;
  people: { id: string; name?: string; isSelf: boolean }[];
  reliable: boolean;
};

/** Map scanned Meet tiles → roster snapshot. Zero tiles = unreliable (DOM not ready / obscured), not empty room. */
export function rosterFromGmeetTiles(
  tiles: { id: string; name: string | null; self: boolean }[],
  tMs: number,
): GmeetRosterSnapshot {
  return {
    tMs,
...
export function createGmeetSpeakers(opts: GmeetSpeakersOptions = {}): GmeetSpeakers {
  ...
  const timer = setInterval(() => {
    ...
    // Live roster (true presence) — every poll; consumer decides publish/leave.
    try { opts.onRoster?.(rosterFromGmeetTiles(tiles, Date.now())); } catch { /* consumer error */ }
  }, pollMs);
