 ...
 */
import { createGmeetSpeakers, rosterFromGmeetTiles } from './gmeet-speakers.js';
...
// ── rosterFromGmeetTiles (Wave 1 true presence) ────────────────────────────────
{
  const empty = rosterFromGmeetTiles([], 100);
  check('roster: [] → unreliable', empty.reliable === false);
  check('roster: [] → no people', empty.people.length === 0);

  const snap = rosterFromGmeetTiles([
    { id: 'b', name: 'Bot', self: true },
    { id: 'a', name: 'Alice', self: false },
    { id: 'c', name: null, self: false },
  ], 200);
  check('roster: reliable when tiles present', snap.reliable === true);
  check('roster: people length 3', snap.people.length === 3);
  check('roster: self flagged', snap.people[0].isSelf === true && snap.people[0].name === 'Bot');
  check('roster: null name → undefined', snap.people[2].name === undefined);
  const humans = snap.people.filter((p) => !p.isSelf).length;
  check('roster: 2 non-self humans', humans === 2);
}

{
  const rosters: ReturnType<typeof rosterFromGmeetTiles>[] = [];
  setDoc(e('body', {}, [
    tile('bot', 'Vexa', { self: true }),
    tile('p1', 'Alice'),
    tile('p2', 'Bob'),
  ]));
  const sp = createGmeetSpeakers({ pollMs: 10, onRoster: (s) => rosters.push(s) });
  tick();
  check('onRoster fires each poll', rosters.length === 1);
  check('onRoster reliable with tiles', rosters[0]?.reliable === true);
  check('onRoster excludes only via isSelf (3 people)', rosters[0]?.people.length === 3);
  sp.destroy();
}

if (failed) { console.error(`\n❌ gmeet-speakers: ${failed} checks FAILED.`); process.exit(1); }
console.log(`\n✅ gmeet-speakers: glow→START/END hint edges, self-tile suppression, junk-name filter, roster pass. (DOM scraping is live-validated.)`);
