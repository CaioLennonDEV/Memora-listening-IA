...
class DockerBackend:
    ...
    def start(
        ...
        # Optional corporate / DPI-SSL CA (HOST_CUSTOM_CA = Docker-host path of a PEM .crt).
        # Bind into every spawned container and set NODE_EXTRA_CA_CERTS so Node fetch (bot STT)
        # trusts the interceptor without replacing the default store.
        custom_ca = os.getenv("HOST_CUSTOM_CA")
        if custom_ca:
            binds.append(f"{custom_ca}:/etc/vexa/certs/custom-ca.crt:ro")
        # Optional overlay of @vexa/transcribe-whisper's client (e.g. Idempotency-Key fix) without
        # rebuilding vexa-bot — Docker-host path to a patched dist/transcription-client.js.
        whisper_client = os.getenv("HOST_BOT_WHISPER_CLIENT", "").strip()
        if whisper_client:
            binds.append(
                f"{whisper_client}:/app/core/meetings/modules/whisper/dist/transcription-client.js:ro"
            )
        # Wave 1/2 live roster: overlay host-built bot dist + page capture bundle without
        # rebuilding vexa-bot (SonicWALL / DPI blocks docker build of the Playwright image).
        # HOST_BOT_DIST_OVERLAY = host dir of bot `dist/` (must include roster.js, index.js, …).
        # HOST_BOT_BROWSER_UTILS = host path to browser-utils.global.js (createGmeetSpeakers onRoster).
        bot_dist = os.getenv("HOST_BOT_DIST_OVERLAY", "").strip()
        if bot_dist:
            binds.append(
                f"{bot_dist}:/app/core/meetings/services/bot/dist:ro"
            )
        bot_browser_utils = os.getenv("HOST_BOT_BROWSER_UTILS", "").strip()
        if bot_browser_utils:
            binds.append(f"{bot_browser_utils}:/app/browser-utils.global.js:ro")
        ...
        spawn_env = dict(env)
        if custom_ca and "NODE_EXTRA_CA_CERTS" not in spawn_env:
            # Node fetch (bot STT → transcription.vexa.ai) trusts DPI/MITM CAs via this path.
            spawn_env["NODE_EXTRA_CA_CERTS"] = "/etc/vexa/certs/custom-ca.crt"
        # Local escape hatch when a corporate DPI CA is present but Node still rejects the chain
        # (incomplete intermediate export). Opt-in only — never default.
        if os.getenv("VEXA_TLS_INSECURE", "").strip().lower() in ("1", "true", "yes"):
            spawn_env.setdefault("NODE_TLS_REJECT_UNAUTHORIZED", "0")
        for key in (
