...
/** Reliable live roster — absent until the first reliable snapshot arrives. */
...
export interface LiveState {
  ...
  /** Live observed humans in the room (Wave 1). Absent ⇒ UI shows "— in the room". */
  roster?: LiveRoster;
}
...
function connect(e: Entry, meetingId: string, sessionUid: string): void {
  ...
  es.onmessage = (m) => {
    let ev: {
      type?: string; speaker?: string; text?: string; t?: number; tsMs?: number; id?: string; completed?: boolean;
      card?: LiveCard; note?: LiveNote; error?: LiveModelError | string; message?: string; status?: number;
      segment_ids?: string[];
      reliable?: boolean; humans?: number; people?: { id: string; name?: string }[]; tMs?: number;
    };
    ...
    if (ev.type === "transcript") {
      ...
      // Unreliable snapshots must not clear a prior reliable count (empty DOM ≠ empty room).
      if (!ev.reliable) return;
      const humans = typeof ev.humans === "number" && Number.isFinite(ev.humans) ? Math.max(0, Math.floor(ev.humans)) : 0;
      s.roster = {
        humans,
        reliable: true,
        tMs: typeof ev.tMs === "number" ? ev.tMs : undefined,
        people: Array.isArray(ev.people) ? ev.people.filter((p) => p && typeof p.id === "string") : [],
      };
    }
