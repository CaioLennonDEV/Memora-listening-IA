...
const NETWORK_HEADLINE = "Couldn't reach the Memora server — check that the stack is running.";
...
export function presentError(e: unknown): PresentedError {
  if (e instanceof ApiError) {
    ...
    if (e.status === 502 || e.status === 504) return { headline: "The Memora server can't reach a backend service right now.", detail };
