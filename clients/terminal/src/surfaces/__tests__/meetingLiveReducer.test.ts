...
describe("meetingLive reducer — live roster (Wave 1)", () => {
  it("ignores unreliable roster and keeps prior reliable humans", () => {
    const { result } = renderHook(() => useMeetingLive("r1", "uid-r1"));
    emit({ type: "roster", reliable: true, humans: 2, tMs: 1, people: [{ id: "a", name: "Alice" }, { id: "b" }] });
    expect(result.current.roster?.humans).toBe(2);
    emit({ type: "roster", reliable: false, humans: 0, people: [] });
    expect(result.current.roster?.humans).toBe(2);
  });

  it("updates humans 2→1 on reliable snapshots", () => {
    const { result } = renderHook(() => useMeetingLive("r2", "uid-r2"));
    emit({ type: "roster", reliable: true, humans: 2, people: [{ id: "a" }, { id: "b" }] });
    emit({ type: "roster", reliable: true, humans: 1, people: [{ id: "a" }] });
    expect(result.current.roster?.humans).toBe(1);
    expect(result.current.roster?.people).toHaveLength(1);
  });
});
