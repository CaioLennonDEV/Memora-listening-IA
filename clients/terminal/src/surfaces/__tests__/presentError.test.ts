...
describe("presentError — ApiError → user truth (fixture range)", () => {
  it("network (status 0) → couldn't reach the server", () => {
    expect(presentError(FIX.network).headline).toBe("Couldn't reach the Memora server — check that the stack is running.");
  });
  it("gateway-down 502 → backend-unreachable truth, NOT the plumbing string", () => {
    const p = presentError(FIX.gatewayDown);
    expect(p.headline).toBe("The Memora server can't reach a backend service right now.");
  ...
  it("upstream-timeout 504 → same backend-unreachable truth", () => {
    expect(presentError(FIX.upstreamTimeout).headline).toBe("The Memora server can't reach a backend service right now.");
  });
...
describe("presentError — non-ApiError failures", () => {
  it("a fetch()-level TypeError reads as the network truth", () => {
    expect(presentError(new TypeError("Failed to fetch")).headline).toBe("Couldn't reach the Memora server — check that the stack is running.");
  });
